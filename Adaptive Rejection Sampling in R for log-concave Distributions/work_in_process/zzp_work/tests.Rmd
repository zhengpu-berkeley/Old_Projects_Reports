...
test_that('check if samples belong to the sampling distribution', {
  ...
  expect_equal( (ks.test(ars(dnorm, 10000), rnorm(10000))$p.value > 0.05) , TRUE)
  expect_equal(ks.test(ars(dexp, 10000, x_domain = c(0, Inf)), rexp(10000))$p.value > 0.05, TRUE)
  expect_equal(ks.test(ars(dgamma, 10000, x_domain = c(0, Inf), shape = 3, scale = 2), 
                       rgamma(10000, shape = 3, scale = 2))$p.value > 0.05, TRUE)
  expect_equal(ks.test(ars(dlogis, 10000), rlogis(10000))$p.value > 0.05, T)
  expect_equal(ks.test(ars(dbeta, 10000, x_domain = c(0, 1), shape1 = 3, shape2 = 2),
                       rbeta(10000, shape1 = 3, shape2 = 2))$p.value > 0.05, T)
  expect_equal(ks.test(ars(dweibull, 1000000, x_domain = c(0, Inf), shape = 2), 
                       rweibull(1000000, shape = 2))$p.value > 0.05, T)
  expect_equal(ks.test(ars(dchisq, 10000, x_domain = c(0, 100), df = 2),
                       rchisq(10000, df = 2))$p.value > 0.05, T)
})
```
...
test_that('check if samples belong to the sampling distribution', {
  # going do the wikipedia page of log-concave distributions
  expect_equal( (chisq.test(ars(dnorm, 10000), rnorm(10000))$p.value > 0.05) , TRUE)
  expect_equal(chisq.test(ars(dexp, 10000, x_domain = c(0, Inf)), rexp(10000))$p.value > 0.05, TRUE)
  expect_equal(chisq.test(ars(dgamma, 10000, x_domain = c(0, Inf), shape = 3, scale = 2), 
                       rgamma(10000, shape = 3, scale = 2))$p.value > 0.05, TRUE)
  expect_equal(chisq.test(ars(dlogis, 10000), rlogis(10000))$p.value > 0.05, T)
  expect_equal(chisq.test(ars(dbeta, 10000, x_domain = c(0, 1), shape1 = 3, shape2 = 2),
                       rbeta(10000, shape1 = 3, shape2 = 2))$p.value > 0.05, T)
  expect_equal(chisq.test(ars(dweibull, 10000, x_domain = c(0, Inf), shape = 2), 
                       rweibull(10000, shape = 2))$p.value > 0.05, T)
  expect_equal(chisq.test(ars(dchisq, 10000, x_domain = c(0, 100), df = 2),
                       rchisq(10000, df = 2))$p.value > 0.05, T)
})
```
...
```{r}
for (i in 1:10){
  expect_equal(ks.test(rlogis(100000), rlogis(100000))$p.value > 0.01, TRUE)
}
