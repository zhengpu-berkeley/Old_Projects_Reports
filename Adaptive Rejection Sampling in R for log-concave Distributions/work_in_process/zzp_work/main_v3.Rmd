...
get_initial_x_vec_and_D = function(target_density, x_domain, ...){
  ...
  if (length(x_vec) < 3) {
    stop(paste('Please Respecify Bounds and Target Density \n',
               'Density Function at Given Domain have Multiple Zero Gradient!'))
  }
  return(return_list)
}
...
get_samples_from_density = function(target_density, x_domain, n, max_iter=10*n, ...){
  ...
  while (curr_num < n && num_iter < max_iter){
    ...
    if (w <= exp(l_k_of(x_star, x_j, x_j_plus_one, ...) - u_k_of(x_star, x_j, ...))){
      ...
      curr_num = curr_num + 1
      samples[curr_num] = x_star
      if ((x_star - x_mid) > 1e-8){
        x_vec = sort(almost.unique(c(x_vec, x_star), tolerance=1e-8))
      }
    } else {
      if ((x_star - x_mid) > 1e-8){
        x_vec = sort(almost.unique(c(x_vec, x_star), tolerance=1e-8))
...
samps = get_samples_from_density(dnorm, c(lower, upper), 10000, mean=mu, sd=std)
...
samps = get_samples_from_density(dnorm, c(lower, upper), 3000, mean=mu, sd=std)
...
samps = get_samples_from_density(dsech, c(lower, upper), 3000, mu=mean, sigma=std)
...
samps = get_samples_from_density(dbeta, c(0, 1), 3000, shape1=shape1, shape2=shape2)
...
samps = get_samples_from_density(dbeta, c(0, 0.15), 3000, shape1=shape1, shape2=shape2)
...
samps = get_samples_from_density(dlogis, c(-Inf, Inf), 3000, location = location, scale =scale)
