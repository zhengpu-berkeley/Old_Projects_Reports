...
# tests to check if samples belong to sampling distribution
# ks test checks goodness of fit to ensure that samples belong
# tests include various log-concave distributions from Wikipedia
# note that these tests are stochastic and may occasionally fail
test_that('check if samples belong to the sampling distribution', {
  expect_equal( (ks.test(ars(dnorm, 1000), rnorm(1000))$p.value > 0.01) , TRUE)
  expect_equal(ks.test(ars(dexp, 1000, x_domain = c(0, Inf)), rexp(1000))$p.value > 0.01, TRUE)
  expect_equal(ks.test(ars(dgamma, 1000, x_domain = c(0, Inf), shape = 3, scale = 2),
                       rgamma(1000, shape = 3, scale = 2))$p.value > 0.01, TRUE)
  expect_equal(ks.test(ars(dlogis, 1000), rlogis(1000))$p.value > 0.01, TRUE)
  expect_equal(ks.test(ars(dbeta, 1000, x_domain = c(0, 1), shape1 = 3, shape2 = 2),
                       rbeta(1000, shape1 = 3, shape2 = 2))$p.value > 0.01, TRUE)
  expect_equal(ks.test(ars(dweibull, 1000, x_domain = c(0, Inf), shape = 2),
                       rweibull(1000, shape = 2))$p.value > 0.01, TRUE)
  expect_equal(ks.test(ars(dchisq, 1000, x_domain = c(0, 100), df = 2),
                       rchisq(1000, df = 2))$p.value > 0.01, TRUE)
})

# tests specifically for auxiliary functions
test_that('some test cases for auxiliary functions', {
  # setting up example problem
  ...
  # test for correct x_vec length
  expect(length(x_vec) > 1, failure_message='x vector wrong')
  # test for correct D length
  expect_equal(length(D),2)
  # test for correct sample length
  expect_equal(length(samps),10)
  # tests approximate equality in 'if_all_unique_append' function
